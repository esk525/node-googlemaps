...
exports.setOption = function setOption(keyname,value){
	switch (keyname){
		case "channel":
		case "proxy":
		case "secure":
		...
		break;
		default:
			throw new Error("Unknown key "+keyname+" in googlemaps::set");
		break;
	}
}
...
function sign(msg,key) {
	var method = 'sha1';	
	var decodedKey = decodeSafe64(key);
	
	var b64 = new Buffer(decodedKey,'base64').toString('binary');		
	var bin = crypto.createHmac(method, b64).update(msg).digest('base64');
	
	var sig = encodeSafe64( bin.toString() );
	
	return sig;    
}
	
function decodeSafe64(str){
	return str.replace(/\-/g, '+').replace(/_/g, '/');
}
function encodeSafe64(str){
...
exports.places = function(latlng, radius, key, callback, sensor, types, lang, name) {
	...
	makeRequest(path, options.secure, returnObjectFromJSON(callback));
};
...
exports.elevationFromPath = function(path , samples , callback , sensor){
	...
	makeRequest(reqPath, options.secure, returnObjectFromJSON(callback));
};
...
exports.staticMap = function(center , zoom , size , callback , sensor ,
	...
	if( typeof( callback ) === 'function' ){
		makeRequest( path , options.secure , callback , 'binary' );
	}
...
var makeRequest = function(path, secure, callback, encoding){
	...
	if (options.clientid){
		xtras.client = options.clientid;		
	}
	if (options.channel){
		xtras.channel = options.channel;
	}
	xtras = qs.stringify(xtras);
	if (xtras != ''){
		path += ('&'+xtras);
	}
	if (options.key){
		var sig =  sign(path,options.key);
		path += ('&signature='+ sig);
	} 
	var url = (secure ? 'https' : 'http') + '://maps.googleapis.com' + path;
	var reqOptions = {
		uri: url
	};
	if( encoding ) {
